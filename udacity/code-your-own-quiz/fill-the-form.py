...
data = {
    'easy': {
        'phrase': '''Hello ___1___!'  In ___2___ this is particularly easy; all
                    you have to do is type in: ___3____ "Hello ___1___!"
    ...
    'medium': {
        'phrase': '''___1___ Inc. is an American multinational technology company headquartered in Cupertino,
                    ...
                    ___2___ personal computer,
                    the iPod portable media player, the ___1___ Watch smartwatch, and the ___1___ TV digital media player.
                     ___1___ 's consumer
                    software includes the macOS and iOS operating systems,
                    the ___3___ media player, the ___4___ web browser, and the iLife and iWork creativity and productivity suites.
                    Its online services include the ___3___ Store, the iOS App Store and ___2___ App Store, ___1___ Music, and iCloud.''',
        'answers': ['Apple', 'Mac', 'iTunes', 'Safari'],
    },
    'hard': {
        'phrase': '''+++Hard+++Hypertext Markup Language (HTML) is the standard markup language for creating ___1___ pages and ___1___ applications.
...
def find_word(word):
    ...
    Output: If the word is in the list, return result and delete the word from the list
    """
...
def check_replacement(string, user_input):
    """
    Behavior: Compare the word in string matched with the word in to_be_changed if so
...
def play_game(answers, string):
    """
    Behavior: In this function we make a counter while , get the user_input, if user_input
    matches with the answer in the list we call function check_replacement() and increase counter.
    If the answer is wrong we count the remain tries. When remain tries == 0 , game is ower.
...
def select_difficulty():
    """
    Behavior: Function asks user to select the difficulty level from (easy/medium/hard)
